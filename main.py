...
@bot.message_handler(func=lambda message: message.text == "💣 MINES")
...
# === Stavka kiritish va o'yinni boshlash ===
def process_mines_bet(message):
    ...
    try:
        ...
        if bet < 1000:
            msg = bot.send_message(user_id, "❌ Minimal tikish 1000 so‘m. Iltimos, qayta kiriting:")
            bot.register_next_step_handler(msg, process_mines_bet)
            return
        ...
        mines = random.sample(range(25), 3)  # 3 ta bomba
        ...
        bot.send_message(
            user_id,
            f"💣 <b>MINES o‘yini boshlandi!</b>\n💵 Tikilgan summa: {bet:,} so‘m\n\n⬜️ 25 ta katakdan 3 tasida 💀 bomba bor.\n💵 xavfsiz kataklarni oching!\n",
            parse_mode="HTML",
            reply_markup=markup
        )

    except:
        msg = bot.send_message(user_id, "❌ Noto‘g‘ri qiymat. Iltimos, faqat raqam kiriting:")
        bot.register_next_step_handler(msg, process_mines_bet)
...
def handle_mines_click(call):
    ...
    if not state or not state['active']:
        bot.answer_callback_query(call.id, "⛔ O'yin mavjud emas yoki tugagan.")
        return
    ...
    if cell_index in state['mines']:
        state['active'] = False
        text = f"💥 Siz 💀 bombani bosdingiz!\n\n"
        text += generate_mines_grid(state)
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id)
        return

    # === Yutuqli katak ochildi ===
    if len(state['opened']) == 22:  # faqat 3 ta bomba bo‘lsa, 22 xavfsiz katak
        state['active'] = False
        win_amount = state['bet'] * 2
        user_balances[user_id] += win_amount

        text = f"🎉 Tabriklaymiz! Siz barcha xavfsiz kataklarni topdingiz!\n💵 Yutuq: {win_amount:,} so‘m\n\n"
        text += generate_mines_grid(state)
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id)
        return

    # === O'yin davom etmoqda ===
    text = f"🧨 Davom eting! Katak ochildi.\n💵 Tikilgan summa: {state['bet']:,} so‘m\n\n"
    text += generate_mines_grid(state)
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id)

# === Emoji bilan kataklarni chizish funksiyasi ===
